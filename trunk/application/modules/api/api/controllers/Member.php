    ...
    {       	
    	...
    	if ($userID) {
    		...
    		if ($data['device_type'] && $data['device_token']) {    			
    			$this->member_model->updateDeviceToken($data['device_token'], $data['device_type'], $userID);
    		}
...
//     	}    
    	...
    	if($device_token && $device_type){
    		//update device info
    		$deviceInfo = $this->member_model->updateDeviceToken($device_token, $device_type, $check_member->id);
        ...
        if (empty ( $user_options )) {
            ...
            if ($latitude && $longitude) {
                $data_option = array_merge ( $data_option, array ( 'location_city' => $latitude . ',' . $longitude ) );
            }
            $this->member_model->add_user_options ( $data_option );
        }
        else{
            if($user_options->location_lock == 'off'){
                if($latitude && $longitude){
                    $data_update_location = array('location_city' => $latitude.','.$longitude);
                    $this->member_model->update_user_options($data_update_location,$member->id);
                }
            }
            else{
                if( empty($user_options->location_city) && $latitude && $longitude){
                    $this->member_model->update_user_options(array('location_city' => $latitude.','.$longitude),$member->id);
                }
            }
        }
   ...
   function listingItems_post() {
        ...
        if (!empty($data['items'])) {
            foreach ($data['items'] as $key => $user) {
                $user_detail = $this->member_model->getMemberByMemberID($user->id , true , true , true , true);
...
//     		}
    	...
    	//update member
    	$this->member_model->update($data,$this->_member->id);
    ...
    public function updateMemberPhoto_post()
    {
    	...
    	if($photo && !empty($photo))
    	{
    	    ...
    	    elseif (isset($_FILES['background'])){
                $data['profile_background'] = $this->config->item('api_upload_path').$this->config->item('member_path').$photo['file_name'];
                $file_name_array = explode('.', $photo['file_name']);
                $data['profile_background_thumb'] = $this->config->item('api_upload_path').$this->config->item('member_path').$file_name_array[0].'_thumb.'.$file_name_array[1];
            }	
        
    		//update data
    		$this->member_model->update($data,$this->_member->id);
    		$response['msg'] = 'Update successful';
            $user = $this->member_model->getMemberByMemberID($this->_member->id , true , true , true , true);
            if(!empty($user))
            {
    ...
    function logout_post()
    {
    	...
    	$this->member_model->logout($this->_member->id, $device_token, $device_type,$member_token);
    ...
    public function addUserCheckin_post()
    {
    	...
    	//add user check in
    	$checkin_id = $this->member_model->add_user_checkin_listing($data);
    	
    	//do activity
//     	$data_activity = array(
//     			'activity_type' => self::PUSH_TYPE_CHECKIN,
...
//     	);
    	...
    	$response = array(
    		'msg'	=> lang('Add user checkin successful')
    	);
    	$this->response($response,200);
    ...
    public function addUserReview_post()
    {
    	...
    	//add user review
    	$review_id = $this->member_model->add_user_review_listing($data);
    	...
    	$listing_detail 			= $this->listing_model->get_listing_detail($data['business_id'] , true , true , true, true , true);
    	$response = array(
    			'msg'	=> lang('Add user review successful'),
    			'item' => $listing_detail
    	);
    	
    	//send push to user who post listing
    	$this->load->model('notification_model');
    	$this->load->helper('notification');    	 
    	$actor_user_id          = $listing_detail->user_id;
    	$name_user_action       = $this->_member->first_name . ' ' . $this->_member->last_name;
    	$message 		= $name_user_action .' '.lang('review on your listing');
    	$action_type            = get_action_type(self::PUSH_TYPE_ADD_REVIEW);
    	$source_id 		= $review_id;
    	$data_push 		= array(
    			'action_type'               => self::PUSH_TYPE_ADD_REVIEW,
    	...
    	//do activity
//     	$data_activity = array(
//     		'activity_type' => self::PUSH_TYPE_ADD_REVIEW,
//     		'source_id'		=> $review_id ,
//     		'created_date'	=> now(),
//     		'is_read'		=> 0,
//     		'user_id'		=> $this->_member->id
...
// 	    			$config['max_height']           = 768;
    ...
    public function userDetail_post()
    {
    	...
    	$user = $this->member_model->getMemberByMemberID($user_id , true , true , true , true);
    	if(!empty($user))
    	{
    		$user = format_output_data($user);
    		//$response['item'] = $user;
...
//     	$keyword 	= $this->post('keyword') ? $this->post('keyword') : false;
    	...
    	if($data['items'])
    	{
    		...
    		{    
    			...
    			//get user info
    			$user_info = $this->member_model->getMemberByMemberID($user_id,true,true,true,true,true);
    ...
    public function userFriends_post()
    {
    	...
    	$friends = $this->member_model->get_user_friends('all',$this->_member->id,$status,'first_name','ASC');
    	if($friends)
    	{
    		foreach($friends as $key => $friend)
    		{
    			$friends[$key] = format_output_data($friend);
    		}
    	}
    	
    	$this->response(array('items'=>$friends),200);
    }
    		...
    		foreach($friends as $friend)
    		{
    			if(isset($friend['email']) && !empty($friend['email']))
    			{
    				...
    				{    					
    					...
    					$name_user_action       = $this->_member->first_name . ' ' . $this->_member->last_name;
    					$message 		= $name_user_action .' send friend request';
    					$data_push 		= array();
...
//     			);
    ...
    public function userAddLikePhoto_post()
    {
    	...
    	//check media
    	$media = $this->member_model->get_media_by_id($data['media_id']);
...
//     				$this->member_model->insert_user_activity($data_activity);
    ...
    public function deleteMedia_post()
    {
    	...
    	//check media
    	$media = $this->member_model->get_media_by_id($media_id);
    	if($media && $media->status == 1)
    	{
    		//delete
    ...
    public function deleteMultiMedia_post()
    {
    	...
    	if(!empty($medias_id)){
    		foreach ($medias_id as $id){
    			//check media
    			$media = $this->member_model->get_media_by_id($id);
    			if($media && $media->status == 1)
    			{
    				//delete
...
//      $keyword    = $this->post('keyword') ? $this->post('keyword') : false;
	...
	public function unfriend_post() {
		...
		$this->member_model->unfriend($friendId, $userId);
