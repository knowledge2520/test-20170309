...
abstract class REST_Controller extends CI_Controller
{
    ...
    */
    ...
    const ERROR_CODE_POST_METHOD           = 1;
    const ERROR_CODE_TOKEN_WRONG           = 2;
    const ERROR_CODE_USER_INVALID          = 3;
    const ERROR_CODE_USER_AUTH_REQUIRED    = 4;
    const ERROR_CODE_USER_AUTH_WRONG       = 5;
    const ERROR_CODE_MISSIG_FIELD_REQUIRED = 6;
    const ERROR_OBJECT_NOT_FOUND           = 404;
    const ERROR_CODE_USER_LOCATION_REQUIRED    = 12;
    const ERROR_CODE_MEMBER_FACEBOOK_REQUIRE_EMAIL      = 13;
    const ERROR_CODE_FILE_ERROR      = 14;
    const ERROR_CODE_UPLOAD_IMAGE_FAIL      = 15;
    
    /*
     * this code for member
    */
    const ERROR_CODE_ITEM_ALREADY_LINKED        = 18;
    const ERROR_CODE_ITEM_NOT_USE               = 19;

    /*
     * this code for member
    */
    const ERROR_CODE_MEMBER_PASSWORD_WRONG       = 7;
    const ERROR_CODE_MEMBER_USERNAME_EMAIL_EXIST = 8;
    const ERROR_CODE_MEMBER_ADD_NEW_WRONG        = 9;
    const ERROR_CODE_MEMBER_USERNAME_NOT_EXIST   = 10;
    const ERROR_CODE_ITEM_NOT_EXIST              = 11;
    const ERROR_CODE_ITEM_IN_USE                 = 17;
    const ERROR_CODE_404                         = 404;
    
    const DISTANCE_DEFAULT_SEARCH                         = 5; //km
    
    const ERROR_CODE_LOCATION_NOT_FOUND = 16;
    
    const PUSH_TYPE_SYSTEM                  = 'SYSTEM_PUSH';
    const PUSH_TYPE_LIKE_PHOTO              = 'LIKE_PHOTO';
    const PUSH_TYPE_LIKE_REVIEW             = 'LIKE_REVIEW';
    const PUSH_TYPE_LIKE_TIP                = 'LIKE_TIP';
    ...
    const PUSH_TYPE_ADD_TIP                 = 'ADD_TIP';
    ...
    const PUSH_TYPE_ADD_MEDIA               = 'ADD_MEDIA';
    const PUSH_TYPE_ADD_COMMENT_TOPIC       = 'ADD_COMMENT_TOPIC';
    const PUSH_TYPE_ADD_COMMENT_REVIEW      = 'ADD_COMMENT_REVIEW';
    const PUSH_TYPE_ADD_COMMENT_CHECKIN     = 'ADD_COMMENT_CHECKIN';
    const PUSH_TYPE_ADD_COMMENT_POSTED      = 'ADD_COMMENT_POSTED';
    const PUSH_TYPE_ADD_COMMENT_PHOTO_LISTING = 'ADD_COMMENT_PHOTO_LISTING';
    const PUSH_TYPE_ADD_COMMENT_PHOTO_SHARING = 'ADD_COMMENT_PHOTO_SHARING';
    ...
    const PUSH_TYPE_BOOKMARK                = 'ADD_BOOKMARK';
    const PUSH_TYPE_SEND_FRIEND_REQUEST     = 'SEND_FRIEND_REQUEST';
    const PUSH_TYPE_ACCEPT_FRIEND_REQUEST   = 'ACCEPT_FRIEND_REQUEST';      
    ...
    public function _remap($object_called, $arguments)
    {
        ...
        if (config_item('rest_enable_keys') && $use_key && !empty($this->rest->key) && !$this->_check_access()) {
            ...
            $this->response(array(('rest_status_field_name') => FALSE, config_item('rest_message_field_name') => 'This API key does not have access to the requested controller.'), 401);
        }

        // Sure it exists, but can they do anything with it?
        if ( ! method_exists($this, $controller_method)) {
            $this->response(array(('rest_status_field_name') => FALSE, config_item('rest_message_field_name') => 'Unknown method.'), 404);
        }
        ...
        if (config_item('rest_enable_keys') && !empty($this->rest->key)) {
            ...
            if (config_item('rest_enable_limits') && !$this->_check_limit($controller_method)) {
                $response = array(('rest_status_field_name') => FALSE, config_item('rest_message_field_name') => 'This API key has reached the hourly limit for this method.');
            ...
            $response = array(('rest_status_field_name') => FALSE, config_item('rest_message_field_name') => 'This API key does not have enough permissions.');
    ...
    public function response($data = NULL, $http_code = NULL, $continue = FALSE)
    {
        ...
        // Otherwise (if no data but 200 provided) or some data, carry on camping!
            ...
            //VuDao custom output
           	/*$my_data = array();
            $my_data['status'] = $http_code == 200 ? true : false;
            ...
            if(is_array($data))
            {
            	if(isset($data['code']))
            	{
            		$my_data['status'] = false;
            	}
            	elseif(!isset($data['code']) && isset($data['status']))
            	{
            ...
            elseif(is_object($data))
            {
            	...
            	elseif(!isset($data->code) && isset($data->code))
            	{
            		$my_data['status'] = $data['status'];
            	}
            	else
            ...
            if (method_exists($this, '_format_'.$this->response->format)) {
                ...
                $output = $this->{'_format_'.$this->response->format}($my_data);
            }
            ...
            elseif (method_exists($this->format, 'to_'.$this->response->format)) {
                ...
                $output = $this->format->factory($my_data)->{'to_'.$this->response->format}();
            }
    ...
    protected function _prepare_digest_auth()
    {
        ...
        if ($digest['response'] != $valid_response) {
            $this->response(array(('rest_status_field_name') => 0, config_item('rest_message_field_name') => 'Invalid credentials'), 401);
            exit;
        }
    }
    ...
    protected function _check_whitelist_auth()
    {
        ...
        if ( ! in_array($this->input->ip_address(), $whitelist)) {
            $this->response(array(('rest_status_field_name') => FALSE, config_item('rest_message_field_name') => 'IP not authorized'), 401);
        }
    }
    ...
    protected function _force_login($nonce = '')
    {
        ...
        $this->response(array(('rest_status_field_name') => FALSE, config_item('rest_message_field_name') => 'Not authorized'), 401);
    }
    ...
    private function _loadAllConfig(){
    	$this->config_items = new stdClass();
    	$query = "SELECT * FROM crm_system_config";
    	...
    	if ($result->num_rows() > 0) {
    		...
    		{
    			$key = $row->key;
    			$this->config_items->$key = $row->value;
    		}
    	}
    	else{
    		// print out system mis
    		show_error('Missing config items',404);
    	}
    }
    protected function _requireAuthToken() {
    	$error = array();
    	$this->load->model('member_model');
    	$method = $this->_detect_method();
    	switch ($method) {
    		case 'post':
    			$memberToken = $this->post('token');
    		break;
    		case 'get':
    			$memberToken = $this->get('token');
    		break;
    		case 'put':
    			$memberToken = $this->put('token');
    		break;
    	}
    
    	if (!$memberToken) {
    		$error['code'] = $this::ERROR_CODE_USER_AUTH_REQUIRED;
    		$error['msg']  = lang('Member Token is required for this action');
    		$this->response($error,406);
    	}
    	$member = $this->member_model->loadMemberFromToken($memberToken);
    	if (empty($member)) {
    		$error['code'] = $this::ERROR_CODE_USER_AUTH_WRONG;
    		$error['msg']  = lang("Member token wrong");
// 			$error['msg']	= "We realise that you're using an older version of Pet Widget. Please download the latest update to enjoy all the new and improved features.";
    		$this->response($error,406);
    	} else {
    		$this->_member = $member;
    		$this->_member->id = $member->user_id;
    	}
    }
    protected function _requireLocation() {
    	$error = array();
    	$lat = $this->post('latitude') ? $this->post('latitude') : false;
    	$long = $this->post('longitude') ? $this->post('longitude') : false;
    
    	if (!$lat || !$long) {
    		$error['code'] = $this::ERROR_CODE_USER_LOCATION_REQUIRED;
    		$error['msg']  = lang('Member location is required for this action');
    		$this->response($error,406);
    	}
    }

    protected function writeApiLog($response = array()) {

        $requestData    = json_encode($this->post());
        $apiUrl         = current_url();
        $requestFile    = json_encode($_FILES);
        $responseData   = json_encode($response);
        $requestMethod  = $this->input->server('REQUEST_METHOD');
        $userAgent      = $this->input->user_agent() . "==||==" . $this->input->ip_address();
        ...
        $this->db->insert("api_log", $arrDataLog);
    }
