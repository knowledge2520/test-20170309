...
class Member_model extends CI_Model {
    ...
    function logout($member_id, $device_token = false, $device_type = false, $member_token = false)
    {
     ...
     *file_name
     */
    public function getMemberByMemberID($id , $get_reviews = false , $get_checkins = false , $get_photos = false , $get_friends = false)
    {
    	$params = array();
    	$query = " SELECT m.id, m.token, m.first_name, m.last_name, m.gender, m.activation_code,"
    			. " m.email, m.phone, m.active, m.last_login, m.profile_photo, m.profile_photo_thumb, m.profile_background, m.profile_background_thumb, m.dob, m.created_on,m.facebook_id";
...
//     			$row->list_reviews	= $list_reviews;
    		...
    		if($get_photos)
    		{
    			$total_photos = $this->get_photos_by_user('count',0,false,$user_id,1, 'id', 'ASC', 'business');
    		...
    		if($get_friends)
    		{
    			...
    			$friends 			= $this->get_user_friends('all',$user_id,1,'first_name','ASC');    			
    			if($friends)
    			{
     ...
     *file_name
     */
	function updateDeviceToken($token, $type, $member_id)
    {    	
...
//     	}
    ...
    public function get_user_friends($option = 'count' , $user_id = false , $status = '' , $order_field = 'id', $order_val = 'ASC')
    {
    	if(!$user_id)
    	{
    		return false;
    	}    	
    	$this->db->select('u.first_name,u.last_name,u.email,u.profile_photo,u.profile_photo_thumb');
...
//     	$this->db->join('business_items as i','i.id=t.business_id');
    ...
    public function get_user_bookmarks($option = 'count' ,$start = 0, $limit = false, $user_id = false , $keyword = false , $order_field = 'id', $order_val = 'ASC')
    {
    	...
    	$business_sql 	= "SELECT * FROM user_bookmarks as b";
    	$business_sql 	.= " WHERE b.user_id = $user_id";
    	//     	echo $business_sql;exit;
    	if($option == 'count')
    	{
    		return $this->db->query($business_sql)->num_rows();
    ...
    public function get_user_checkins($option = 'count' ,$start = 0, $limit = false, $user_id = false , $keyword = false , $order_field = 'id', $order_val = 'ASC')
    {
    	...
    	$business_sql 	= "SELECT * FROM user_checkins as c";
    	$business_sql 	.= " WHERE c.user_id = $user_id";
    	//     	echo $business_sql;exit;
    	if($option == 'count')
    	{
    		return $this->db->query($business_sql)->num_rows();
    ...
    public function get_user_contact($option = 'count' , $order_field = 'id', $order_val = 'DESC' ,$user_id = false , $social_type = '' , $is_registed = false  )
    {
    	...
    	$this->db->from('user_contact as c');
    	$this->db->join('users as u', 'u.email = c.email','left');
    	$this->db->where('c.user_id',$user_id);
    	if($social_type!='')
    	{
    ...
    public function addNewUserFriendContact($user_id,$friend_email){
        $user_friend = $this->getMemberByEmail($friend_email);
        if($user_friend){
            $data = array(
                'user_id' => $user_id,
                'email' => $friend_email,
    ...
    {
    	return $this->db->get_where('user_media',array('id'=>$media_id))->first_row();
    }
    ...
    public function get_user_options($user_id){
    	$row = $this->db->get_where('user_options',array('user_id'=>$user_id) )->first_row();
    	return $row;
    }
	...
	public function getTotalFriendRequest($userId) {
		 ...
		 */
		$query = $this->db->get_where("user_contact", array("registed" => $userId, "status" => 3));
