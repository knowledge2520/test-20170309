...
class Site extends REST_Controller
{
    ...
    public function index_get()
    {
    	...
    	//get pet types
    	$resultPetType = $this->db->query("SELECT * FROM pet_types WHERE `status` = 1 ORDER BY `order` ASC;")->result();
    	if($resultPetType)
    	{
...
//         );
		...
		if ($user_options) {
			$option_lock = $user_options->location_lock;
			$user_location_latitude = ! empty ( $user_options->location_city ) && explode ( ',', $user_options->location_city ) [0] ? trim ( explode ( ',', $user_options->location_city ) [0] ) : $latitude;
			$user_location_longitude = ! empty ( $user_options->location_city ) && explode ( ',', $user_options->location_city ) [1] ? trim ( explode ( ',', $user_options->location_city ) [1] ) : $longitude;
			
			if ($option_lock == 'on') {
				if ($latitude && $longitude && empty ( $user_options->location_city )) {
					$this->member_model->update_user_options ( array (
    	...
    	$latest_listings = array();//$this->listing_model->search_listings('all',0,4,false,$user_location,false,'id','DESC');
    	...
    	if($latest_listings)
    	{
    		foreach($latest_listings as $key => $listing)
    		{
    			...
    			//$placeholder = '../assets/images/uploads/placeholder.png'
    			$listing_photo = (!empty($listing_detail->photo) && file_exists($listing_detail->photo_path)) ? $listing_detail->photo : "";
    			$listing_detail->photo = $listing_photo;

    			///home/appscyclone/User/Dropbox/Project/pet-server/trunk/assets/images/uploads/placeholder.png
                $listing_detail->photo_width = !empty($listing_photo) && fileExists(getimagesize($listing_detail->photo_path)[0]) ? getimagesize($listing_detail->photo_path)[0] : 0;
                $listing_detail->photo_height = !empty($listing_photo) && fileExists(getimagesize($listing_detail->photo_path)[1]) ? getimagesize($listing_detail->photo_path)[1] : 0;
                ...
                unset($listing_detail->photo_path);
    	...
    	$hot_topic = array();//$this->pettalk_model->get_hot_topic(0,4);
    	if($hot_topic){
    	...
    	$latest_review = array();//$this->review_model->get_reviews_by_distance('all',0,4,'id','DESC', $user_location);
    	if($latest_review){
