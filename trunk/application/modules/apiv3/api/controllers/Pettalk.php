...
class Pettalk extends REST_Controller {
    ...
    function addTopic_post() {
        ...
        $catId      = $this->post('category_id') ? $this->post('category_id') : false;
        ...
        $result = $this->pettopic->saveNew( $title, $content, $catId, $userTag, $this->_member );
    ...
    public function list_post() {
        ...
        $response = $this->pettopic->categoryList();
        $this->response($response, 200);
...
//        $data['items'] = $items;
 //       $this->response($data, 200);
    ...
    function updateTopic_post(){
        ...
        $topicId    = $this->post('id') ? $this->post('id') : false;
        $title      = $this->post('title') ? $this->post('title') : false;
        $content    = $this->post('content') ? $this->post('content') : false;
        $catId      = $this->post('category_id') ? $this->post('category_id') : false;
        $userTag    = $this->post('userTag') ? $this->post('userTag') : "";
        ...
        $data[ITEM] = $this->pettopic->save( $newsFeedId, $topicId, $this->_member, $title, $content, $catId, $removeMedia, $userTag );
        $this->response($data, 200);
    }
...
//                    $media_insert['topic_id'] = $data['topic_id'];
            ...
            if( $this->_member->id != $topic->created_by ) {
    ...
    public function addLike_post() {
        ...
        if ($check_like) {
            ...
            if ($this->member_model->add_like($data)) {
                ...
                if( $response['item']->created_by != $this->_member->id ) {
    ...
    /**
     * @description: Add new pet adoption
     */
    public function addAdoption_post() {
        $this->_requireAuthToken();

        $params         = array();

        $params['catId']          = $this->post('category_id') ? $this->post('category_id') : false;
        $params['name']           = $this->post('name') ? $this->post('name') : "";
        $params['age']            = $this->post('age') ? $this->post('age') : "";
        $params['type']           = $this->post('type') ? $this->post('type') : "";
        $params['breed']          = $this->post('breed') ? $this->post('breed') : "";
        $params['sex']            = $this->post('sex') ? $this->post('sex') : "";
        $params['color']          = $this->post('color') ? $this->post('color') : "";
        $params['contact']        = $this->post('contact') ? $this->post('contact') : "";
        $params['location']       = $this->post('location') ? $this->post('location') : "";
        $params['lat']            = $this->post('latitude')  && is_numeric ($this->post('latitude')) ? $this->post('latitude') : "0.0";
        $params['lng']            = $this->post('longitude') && is_numeric ($this->post('longitude')) ? $this->post('longitude') : "0.0";
        $params['additionalInfo'] = $this->post('additionalInfo') ? $this->post('additionalInfo') : "";
        $userTag                  = $this->post('tags') ? $this->post('tags') : "";
        $params['infoType']       = ADD_PETTALK_ADOPTION;
        $params['sex']            = !empty($params['sex']) ? strtolower($params['sex']) : "";
        /*
         * $when           = $this->post('when') ? $this->post('when') : false;
        $where          = $this->post('where') ? $this->post('where') : false;
        $rewardCurrency = $this->post('rewardCurrency') ? $this->post('rewardCurrency') : false;
        */
        ...
        $data[ITEM] = $this->pettopic->saveNewPetInfo( $params, $this->_member, $userTag );

        $this->response($data, 200);
    }

    /**
     * @description: Add new pettalk lost report
     */
    public function addLostReport_post() {
        $this->_requireAuthToken();

        $params         = array();

        $params['catId']          = $this->post('category_id') ? $this->post('category_id') : false;
        $params['when']           = $this->post('when') ? $this->post('when') : "";
        $params['where']          = $this->post('where') ? $this->post('where') : "";
        $params['contact']        = $this->post('contact') ? $this->post('contact') : "";
        $params['rewardCurrency'] = $this->post('rewardCurrency') ? $this->post('rewardCurrency') : "";
        $params['currency']       = $this->post('currency') ? $this->post('currency') : "";
        $params['name']           = $this->post('name') ? $this->post('name') : "";
        $params['type']           = $this->post('type') ? $this->post('type') : "";
        $params['breed']          = $this->post('breed') ? $this->post('breed') : "";
        $params['age']            = $this->post('age') ? $this->post('age') : "";
        $params['sex']            = $this->post('sex') ? $this->post('sex') : "";
        $params['color']          = $this->post('color') ? $this->post('color') : "";
        $params['microchip']      = $this->post('microchip') ? $this->post('microchip') : "";
        $params['lat']            = $this->post('latitude')  && is_numeric ($this->post('latitude')) ? $this->post('latitude') : "0.0";
        $params['lng']            = $this->post('longitude') && is_numeric ($this->post('longitude')) ? $this->post('longitude') : "0.0";
        $params['additionalInfo'] = $this->post('additionalInfo') ? $this->post('additionalInfo') : "";
        $userTag                  = $this->post('tags') ? $this->post('tags') : "";
        $params['infoType']       = ADD_PETTALK_LOST_REPORT;
        $params['sex']            = !empty($params['sex']) ? strtolower($params['sex']) : "";
        ...
        $_POST = $this->post(); //set this for validate
        $this->form_validation->set_rules('category_id', 'Category', 'required');
        //$this->form_validation->set_rules('when', 'When', 'required');
    ...
    public function addFoundReport_post() {
        $this->_requireAuthToken();
        ...
        $params['additionalInfo'] = $this->post('additionalInfo') ? $this->post('additionalInfo') : "";
        $userTag                  = $this->post('tags') ? $this->post('tags') : "";
        $params['infoType']       = ADD_PETTALK_FOUND_REPORT;
        $params['sex']            = !empty($params['sex']) ? strtolower($params['sex']) : "";
    ...
    public function getPettalkInfoList_post() {
        $this->_requireAuthToken();

        $catId      = $this->post('catId') ? $this->post('catId') : false;
        $keyword    = $this->post('keyword') ? $this->post('keyword') : false;
        ...
        /*if( !$infoId ) {
            $error['msg'] = "Please input item ID";
            $error['code'] = self::ERROR_CODE_MISSIG_FIELD_REQUIRED;
            $this->response($error, 200);
        }*/

        //$userId = $userId ? $userId : $this->_member->id;

        /*
         * Note: This API get all pettalk of all users so that we may not need userId at this time
         * But we have to pass logged in user ID to check the feed item has liked by this user or not
         */
        $response = $this->pettopic->pettalkList( $userId, $catId, $keyword, $this->_member, $start, $limit );
    ...
    public function search_post() {
        ...
        $response = $this->pettopic->searchPetTalk($this->_member, $keyword, 'items', $limit, $start,$user_location);
        $this->response($response, 200);
    }
}
