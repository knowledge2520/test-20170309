...
// 		$this->_requireAuthToken();
    ...
    function listingItems_post() {
        ...
        $keyword = $this->post('keyword') ? $this->post('keyword') : false;
        $sort_by = $this->post('sort_by') ? $this->post('sort_by') : 'newest';
    ...
    public function listingDetail_post() {
        ...
        if( $lat && $long ) {
            $this->friendnearby->setUserLocation( $this->_member, $lat, $long );
        }
        ...
        if ($listing_detail) {
            ...
            if ((!empty($lat) && !empty($long)) AND ( !empty($listing_detail->latitude) && !empty($listing_detail->longitude))) {
                $distance = $this->listing_model->calculate_distance($lat, $listing_detail->latitude, $long, $listing_detail->longitude);
    ...
    public function addMediaListing_post() {
        ...
        $listingId = $this->post('listing_id') ? $this->post('listing_id') : 0;
        ...
        $response[ITEM] = $this->petlisting->saveNew( $listingId, $this->_member, $userTag );
        $this->media_model->updateCoverListing(array($listingId));
        $this->response($response, 200);
    }
...
//     	$data['longitude'] 		= $this->post('longitude') ? $this->post('longitude') : false;
        ...
        if (empty($location)) {
            ...
            $country = $this->listing_model->get_country_by('countryname', $data['country']);
        ...
        if ($listing_id) {
            ...
            $data_insert = $this->media_model->saveMedia( false, $listing_id, 'business_id', $this->_member->id );
            //update listing photo
            if (!empty($data_insert)) {
                $this->listing_model->editListing(array('photo' => $data_insert[0]['source']), $listing_id);
            }
    ...
    public function getReviewsByListing_post() {
        ...
        if ($reviews) {
            foreach ($reviews as $key => $review) {
                $review_detail = $this->review_model->get_review_detail($review->id, true, true, true, true, true, true);
...
//     			$listing_detail->distance 	= $listing->distance;
    ...
    public function search_post(){
        ...
        $data['items'] = $this->listing_model->search_listings_category('all', $start, $limit, $user_location, $keyword);
        $data['totalItem'] = $this->listing_model->search_listings_category('count', $start, $limit, $user_location, $keyword);
        $data['totalPage'] = ceil(intval($data['totalItem']) / $limit);
        $data['limit'] = intval($limit);
