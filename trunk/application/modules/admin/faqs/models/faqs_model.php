...
class Faqs_model extends MY_Model
{
     ...
     *file_name
     */
    public function getItems($option = 'total' , $status = '' , $keyword = '' , $order_field = 'id' , $sort = 'ASC'  ,$limit = ADMIN_ITEMS_PERPAGE , $offset = false){
        if($status){
            $this->where('status', $status);
        }
        if($keyword !='')
        {
            $this->like('question',$keyword);
            $this->or_like('answer',$keyword);
        }
