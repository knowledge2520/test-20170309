...
{
    protected $table_name	= "user_likes";
    protected $key			= "id";
    ...
    public function getItems($option = 'total' , $status = 0 , $keyword = '' , $order_field = 'id' , $sort = 'ASC'  ,$limit = ADMIN_ITEMS_PERPAGE , $offset = false){
        ...
        if($keyword !='')
        {
            $user_keyword = $this->searchUsers($keyword);
            $business_keyword = $this->searchBusiness($keyword);

            if($user_keyword){
                foreach($user_keyword as $k){
    ...
    function deleteUserLikeBy($field, $value){
        if(!$field || !$value){
            return false;
        }
        $this->db->delete('user_likes', array($field => $value));
    }
