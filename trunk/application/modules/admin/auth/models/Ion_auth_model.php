...
class Ion_auth_model extends CI_Model
{
    ...
    public function hash_password_db($id, $password, $use_sha1_override=FALSE)
    {
        ...
        if ($this->store_salt)
        {
            $db_password = sha1($hash_password_db->salt . $password);
        }
        else
        {
    ...
    public function email_check($email = '')
    {
        ...
        $query = "SELECT * FROM ".$this->tables['users']." WHERE `email`='".$email."' AND (`active` = 1 OR `active` = 0)";
        $results = $this->db->query($query);
        return !empty($results->result()) ? TRUE : FALSE;
...
//             ->count_all_results($this->tables['users']) > 0;
     ...
     * @author Mathew
     **/
    public function register($username, $password, $email, $additional_data = array(), $groups = array() , $countries = false)
    {
        ...
        if($countries){
            ...
            foreach ($countries as $country)
            {
                ...
                $this->db->insert('business_users_country',$dataInsert);
            }
        }
    ...
    public function addCountry($userId,$countries){
        if(count($countries)>0){
            $query = "DELETE from business_users_country where user_id='$userId'";
            $this->db->query($query);
            //add to countries
            foreach ($countries as $country)
            {
