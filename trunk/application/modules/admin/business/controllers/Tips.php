...
class Tips extends Admin_Controller {
    ...
    var $data = array();
    var $module = 'business.tips';

    function __construct() {
        parent::__construct();

        $this->load->library(array('messages'));
        $this->load->model(array('tips_model', 'business_model', 'users/users_model', 'media_model'));
    ...
    function index($business_id = false) {
        if (!Permission::check_permission($this->module . '.index') && !$this->ion_auth->is_admin() && !Permission::check_permission('business.individual')) {
            ...
            $status = array(0, 1);//$this->input->get('status') ? $this->input->get('status') : false;
            ...
            $array_field = array('id', 'full_name', 'business_name', 'description', 'created_date');
            ...
            //get data
            $this->data['total'] = $this->tips_model->getItems('total', $status, $keyword, false, false, $limit, $offset, $business_id, $my_tip, $this->session->userdata('user_id'));
            $this->data['total_deactivate'] = $this->tips_model->getItems('total', array(0), $keyword, false, false, $limit, $offset, $business_id, $my_tip, $this->session->userdata('user_id'));
            $this->data['records'] = $this->tips_model->getItems('list', $status, $keyword, $order_field, $sort, $limit, $offset, $business_id, $my_tip, $this->session->userdata('user_id'));
            $this->data['count'] = $this->tips_model->getItems('count_list', $status, $keyword, $order_field, $sort, $limit, $offset, $business_id, $my_tip, $this->session->userdata('user_id'));
        ...
        if($this->input->post('set_status')){            
            ...
            $response = array();
            $this->tips_model->update_status($id, $status);
        ...
        if ($this->input->post('btn_delete')) {
            ...
            if (is_array($checked) && count($checked)) {
                ...
                foreach ($checked as $id) {
                    ...
                    $result = $this->tips_model->deleteTip($id);
                    if ($result) {
                        $this->messages->add(lang('tip_delete_action_success', $id), "success");
                        //log_message('message', 'deleted successful business item id:'.$id);
                    } else {
                        $this->messages->add(lang('tip_delete_action_fail', $id), "error");
...
//                    }
            ...
            if ($this->input->post()) {
                ...
                if ($id) {
                    ...
                    if (!Permission::check_permission($this->module . '.edit') && !$this->ion_auth->is_admin()) {
                        redirect($this->lang->lang() . '/business/tips/');
                    } else {
    ...
    public function edit($id) {
        if (!Permission::check_permission($this->module . '.edit') && !$this->ion_auth->is_admin()) {
            ...
            $this->data['record']->user = $this->users_model->detail($this->data['record']->user_id);
            $this->data['record']->business = $this->business_model->detail($this->data['record']->business_id);
    ...
    public function mediaList($tip_id = false) {
        if (!Permission::check_permission($this->module . '.edit') && !$this->ion_auth->is_admin()) {
            ...
            //get data
            $this->data['total'] = $this->media_model->getItems('total', $status, $keyword, false, false, $limit, $offset, 'm.tip_id', $tip_id);
            $this->data['records'] = $this->media_model->getItems('list', $status, $keyword, 'id', 'DESC', $limit, $offset, 'm.tip_id', $tip_id);
            $this->data['count'] = $this->media_model->getItems('count_list', $status, $keyword, 'id', 'DESC', $limit, $offset, 'm.tip_id', $tip_id);
    ...
    private function _save_media($type = 'insert', $tip_id = false, $media_id = 0) {
        ...
        if ($type == 'insert') {
            ...
            //load banner data
            $banner = $this->media_model->find($media_id);
